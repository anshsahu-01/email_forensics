  ...
  if (loading) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-white">
        ...
        <p className="mt-6 text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Deciphering artifact...</p>
      </div>
    );
  }
  ...
  if (error || !emailCase) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-white px-8 text-center">
        ...
        <h1 className="mt-8 text-2xl font-black uppercase tracking-widest text-slate-900">Access Denied</h1>
        <p className="mt-4 text-sm font-medium text-slate-500 max-w-md uppercase tracking-tight">{error || 'The requested forensic report could not be retrieved.'}</p>
        <Link
          href="/cases"
        ...
        <div className="mx-auto max-w-7xl px-8 py-6">
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-6">
              <Link
                href="/cases"
                className="flex h-12 w-12 items-center justify-center border-2 border-slate-200 bg-white text-slate-400 hover:border-slate-900 hover:text-slate-900 transition-all"
              >
              ...
              <div>
                ...
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 mt-1">
                  Investigation log active since {new Date(emailCase.createdAt).toLocaleString()}
                </p>
              </div>
            </div>

            <div className="flex items-center gap-4">
              <button className="inline-flex items-center gap-2 border-2 border-slate-200 bg-white px-5 py-2.5 text-[10px] font-black uppercase tracking-widest text-slate-900 hover:border-slate-900 transition-all">
      ...
      <div className="mx-auto max-w-7xl px-8 py-12">
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-3">
                ...
                <div className="grid gap-8 sm:grid-cols-2">
                  <div className="border border-slate-200 bg-white">
                    ...
                    <div className="p-8 space-y-6">
                      <div>
                        <p className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 mb-2">Network Origin IP</p>
                        <p className="font-mono text-sm font-black text-slate-900 bg-slate-50 p-3 border border-slate-100">
                          {emailCase.originatingIp || 'DATA NOT DETECTED'}
                        </p>
                      </div>
                      <div className="grid grid-cols-2 gap-6">
                        <div>
                          <p className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 mb-2">Geo-Location</p>
                          <p className="flex items-center gap-2 text-[11px] font-bold text-slate-900 uppercase tracking-widest">
                            <MapPin className="h-3.5 w-3.5 text-slate-300" />
                            GLOBAL / CLOUD
                          </p>
                        </div>
                        <div>
                          <p className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 mb-2">ASN Registry</p>
                          <p className="text-[11px] font-bold text-slate-900 uppercase tracking-widest">UNDEFINED</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="border border-slate-200 bg-white">
                    ...
                    <div className="p-8 space-y-6">
                      <div>
                        <p className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 mb-2">SHA-256 Digest</p>
                        <p className="font-mono text-[11px] font-bold text-slate-600 break-all bg-slate-50 p-4 border border-slate-100 leading-relaxed">
          ...
          <div className="space-y-10">
            <div className="border border-slate-200 bg-white">
              <div className="border-b border-slate-200 px-8 py-4 bg-slate-50">
                <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-900">Risk Assessment</h3>
              </div>
              <div className="p-8 space-y-8">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Threat Matrix Score</span>
                  <span className="text-lg font-black text-slate-900">{emailCase.threatScore ?? 0}<span className="text-slate-300 text-xs ml-1">/100</span></span>
                </div>
                <div className="h-1.5 w-full bg-slate-100 overflow-hidden">
                  <div
                    className={`h-full transition-all duration-1000 ${
                      (emailCase.threatScore ?? 0) > 70 ? 'bg-red-600' : (emailCase.threatScore ?? 0) > 40 ? 'bg-amber-600' : 'bg-emerald-600'
                    }`}
            ...
            <div className="border border-slate-200 bg-white">
              <div className="border-b border-slate-200 px-8 py-4 bg-slate-50">
                <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-900">Indicator Matrix</h3>
              </div>
              <div className="p-8 space-y-6">
                <IndicatorStat label="MALICIOUS ENDPOINTS" count={emailCase.indicators?.filter(i => i.type === 'URL' && (i.vtMalicious ?? 0) > 0).length ?? 0} color="text-red-600" />
                <IndicatorStat label="SUSPICIOUS INFRASTRUCTURE" count={0} color="text-amber-600" />
                <IndicatorStat label="PROTOCOL FAILURES" count={[emailCase.header?.spfStatus, emailCase.header?.dkimStatus, emailCase.header?.dmarcStatus].filter(s => s === 'fail').length} color="text-red-600" />
              </div>
            </div>

            <div className="bg-slate-900 p-8 text-white">
              <Info className="h-8 w-8 mb-6 text-indigo-400" />
              <h3 className="text-xs font-black uppercase tracking-[0.3em] mb-4">Forensic Triage Complete</h3>
...
function IndicatorStat({ label, count, color }: { label: string, count: number, color: string }) {
  return (
