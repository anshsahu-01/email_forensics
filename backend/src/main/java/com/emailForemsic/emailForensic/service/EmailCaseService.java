...
public class EmailCaseService {
    ...
    public EmailCase processAndSaveEml(MultipartFile file) throws Exception {
        ...
        try (InputStream inputStream = new ByteArrayInputStream(fileBytes)) {
            ...
            // Determine the best IP to enrich with GeoLocation / ASN / AbuseIPDB / RDAP.
            ...
            if (enrichmentIp != null) {
                ...
                try {
                    ...
                    abuseResult = AbuseIpDbResult.builder()
                            .status("ERROR")
                            .build();
                }
                ...
                // RDAP / Registry Intelligence lookup — non-fatal.
                // RDAP provides registry ownership and IP allocation information.
                // RDAP does not affect the threat score.
                RdapResult rdapResult;

                try {
                    rdapResult = rdapService.lookup(enrichmentIp);
                } catch (RuntimeException ex) {
                    // Unexpected runtime failure — degrade gracefully
                    rdapResult = RdapResult.builder().build();
                }

                ipIndicator.setRdapServer(rdapResult.getRdapServer());
                ipIndicator.setRdapRegistry(rdapResult.getRegistry());
                ipIndicator.setRdapHandle(rdapResult.getHandle());
                ipIndicator.setRdapName(rdapResult.getName());
                ipIndicator.setRdapOrganization(rdapResult.getOrganization());
                ipIndicator.setRdapCountry(rdapResult.getCountry());
                ipIndicator.setRdapStartAddress(rdapResult.getStartAddress());
                ipIndicator.setRdapEndAddress(rdapResult.getEndAddress());
                ipIndicator.setRdapCidr(rdapResult.getCidr());
            ...
            if (parsedResult.getExtractedUrls() != null) {
                ...
                for (String url : parsedResult.getExtractedUrls()) {
                    ...
                    try {
                        ...
                        reputation = VirusTotalReputationResult.builder()
                                .status("ERROR")
                                .build();
                    }
    ...
    private void calculateAndSetThreatScore(EmailCase emailCase) {
        ...
        if (emailCase.getHeader() != null) {
            ...
            if ("fail".equalsIgnoreCase(emailCase.getHeader().getSpfStatus())) {
                score += 10;
            }

            if ("fail".equalsIgnoreCase(emailCase.getHeader().getDkimStatus())) {
                score += 10;
            }

            if ("fail".equalsIgnoreCase(emailCase.getHeader().getDmarcStatus())) {
                score += 10;
            }
        }
    ...
    private String serializeReceivedHeaders(EmailParsedResult parsedResult)
    ...
    private void analyzeSpoofingRisk(
        ...
        String dmarcStatus = parsedResult.getDmarcStatus() != null
        ...
        boolean authIndicatesFailure =
                "fail".equalsIgnoreCase(parsedResult.getSpfStatus())
                        || "softfail".equalsIgnoreCase(parsedResult.getSpfStatus())
